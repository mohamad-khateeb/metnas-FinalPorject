import { addCourse, getAllCourses, getCourseDoc, deletecourse, getUserDoc, deleteuser ,setCoursearr} from './firebase.js'
...
$(document).ready(function () {
    ...
    var display = $("#tbody");
    display.empty();
    const data = getCourseDoc(courseid);
    data.then((dt) => {
        var st = "מצב האשמה לקורס - ";
        st=st+""+ dt.coursename;
        $('#statusReg').text(dt.coursename);
        course_data = dt;
        var waitig_list = dt.waiting;
        var number = 1;
        for (const x of waitig_list) {
           
            const userdoc = getUserDoc(x)
                .then((data) => {
                    ...
                    //     birthday = data.birthday,
                    //     gender = data.gender;
                    htmlToString(display, data.id, data, number)
                    number += 1;
                });
        }
    });
    ...
    $('body').on('click', '#deletebtn', function () {
        ...
        if (index !== -1) {
            arr_waiting.splice(index, 1);
        }
        course_data.waiting = arr_waiting;
        const status_course = setCoursearr(courseid, course_data)
            .then((dt) => {
